...
public sealed class DspDownloader
{
    ...
    public DspDownloader(INexusService nexus, HttpClient http, DspPaths paths)
    {
        _nexus = nexus;
        _http = http;
        _paths = paths;
    ...
    {
        var files = await _nexus.GetFilesAsync(DspNexusCatalog.GameSlug, modId, ct);
        ...
        var url = await _nexus.GetDownloadLinkAsync(DspNexusCatalog.GameSlug, modId, primary.FileId, ct);
        if (url is null)
        {

...
/// <see cref="DspInstallManifestStore"/> (Installed-Rows, wo die ModId
...
public sealed class DspNexusRowEnricher
{
    ...
    public DspNexusRowEnricher(INexusService nexus, CoverCache covers, IHostServices host)
    ...
    public async Task EnrichAsync(IDspEnrichableRow row, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            try
            {
                if (detail is null)
                {
                    detail = await _nexus.GetModDetailAsync(DspNexusCatalog.GameSlug, modId, ct);
    ...
    public async Task EnrichBatchAsync(IEnumerable<IDspEnrichableRow> rows, CancellationToken ct = default)
    {
...
public interface IDspEnrichableRow
{
    int? NexusModId { get; }
